# World Scale Generator - Complete Implementation
# Generates 128×96 world with plate tectonics, elevation, climate, rivers, and biomes
...
@dataclass
class WorldTile:
    """Complete data for one world tile"""
    # Coordinates
    x: int
    y: int
...
class WorldScaleGenerator:
    ...
    def _generate_river_systems(self):
        ...
        # Step 1: Calculate drainage directions using steepest descent
        for y in range(self.world_height):
            for x in range(self.world_width):
                tile = self.world_tiles[y][x]
                
                if not tile.is_land:
                    continue  # No drainage for ocean tiles
                ...
                for dy in [-1, 0, 1]:
                    for dx in [-1, 0, 1]:
                        ...
                        nx, ny = x + dx, y + dy
                        if 0 <= nx < self.world_width and 0 <= ny < self.world_height:
                            neighbor = self.world_tiles[ny][nx]
                            elevation_diff = tile.final_elevation - neighbor.final_elevation
                            
                            # Account for distance (diagonal vs orthogonal)
                            distance = math.sqrt(dx*dx + dy*dy)
                ...
                tile.drainage_direction = best_direction
        
        # Step 2: Calculate flow accumulation
        for y in range(self.world_height):
            for x in range(self.world_width):
                self._calculate_flow_accumulation(x, y, set())
        
        # Step 3: Identify major rivers
        river_threshold = self.world_width * self.world_height * 0.001  # Rivers need significant flow
        river_id = 0
        ...
        for y in range(self.world_height):
            for x in range(self.world_width):
                ...
                if (tile.is_land and 
                    tile.water_accumulation > river_threshold and
                    not tile.has_major_river):
                    
                    # Trace river downstream
                    self._trace_river(x, y, river_id)
                    river_id += 1
                    
                    if river_id >= self.num_major_rivers:
                        break
            if river_id >= self.num_major_rivers:
                break
    
    def _calculate_flow_accumulation(self, x: int, y: int, visited: Set[Tuple[int, int]]) -> float:
        """Recursively calculate flow accumulation for a tile"""
        if (x, y) in visited:
            return 0.0  # Avoid cycles
        
        tile = self.world_tiles[y][x]
        
        if tile.water_accumulation > 0:
            return tile.water_accumulation  # Already calculated
        
        visited.add((x, y))
        
        # Base contribution (rainfall)
        accumulation = tile.precipitation * 10  # Base flow from precipitation
        
        # Add contributions from upstream tiles
        for dy in [-1, 0, 1]:
            for dx in [-1, 0, 1]:
                if dx == 0 and dy == 0:
                    continue
                ...
                if (0 <= nx < self.world_width and 
                    ...
                    # Check if neighbor drains to this tile
                    if (neighbor.drainage_direction and
                        neighbor.drainage_direction[0] == -dx and
                        neighbor.drainage_direction[1] == -dy):
                        
                        upstream_flow = self._calculate_flow_accumulation(nx, ny, visited.copy())
                        accumulation += upstream_flow
        
        tile.water_accumulation = accumulation
        visited.discard((x, y))
        return accumulation
    
    def _trace_river(self, start_x: int, start_y: int, river_id: int):
        ...
        while True:
            ...
            tile = self.world_tiles[current_y][current_x]
            
            tile.has_major_river = True
            tile.river_id = river_id
            
            # Follow drainage direction
            if not tile.drainage_direction:
                break  # No downstream direction
            
            dx, dy = tile.drainage_direction
            next_x, next_y = current_x + dx, current_y + dy
            
            if not (0 <= next_x < self.world_width and 0 <= next_y < self.world_height):
                break  # Reached edge
            
            next_tile = self.world_tiles[next_y][next_x]
            
            if not next_tile.is_land:
                # Reached ocean
                next_tile.has_major_river = True  # River mouth
                next_tile.river_id = river_id
                break
    ...
    def _generate_display_chars(self):
        ...
        biome_display = {
            # Ocean
            BiomeType.DEEP_OCEAN: ("~", (50, 100, 200), (0, 50, 100)),
            BiomeType.SHALLOW_SEA: ("≈", (100, 150, 255), (25, 75, 150)),
            BiomeType.COASTAL_WATERS: ("∼", (150, 200, 255), (50, 100, 200)),
            
            # Cold
            BiomeType.POLAR_ICE: ("*", (240, 240, 255), (200, 200, 220)),
            BiomeType.TUNDRA: (".", (180, 180, 200), (120, 120, 140)),
            BiomeType.TAIGA: ("♠", (0, 100, 0), (0, 60, 0)),
            
            # Temperate
            BiomeType.TEMPERATE_FOREST: ("♣", (0, 120, 0), (0, 80, 0)),
            BiomeType.GRASSLAND: ("\"", (80, 150, 80), (40, 100, 40)),
            
            # Hot
            BiomeType.TROPICAL_FOREST: ("♠", (0, 150, 0), (0, 100, 0)),
            BiomeType.SAVANNA: ("'", (120, 150, 60), (80, 100, 40)),
            BiomeType.DESERT: ("·", (218, 165, 32), (184, 134, 11)),
            
            # Mountain
            BiomeType.HIGH_MOUNTAINS: ("▲", (200, 180, 160), (120, 100, 80)),
            BiomeType.MOUNTAIN_FOREST: ("^", (80, 120, 80), (40, 80, 40)),
            BiomeType.MOUNTAIN_DESERT: ("^", (180, 140, 100), (140, 100, 60)),
            
            # Wetland
            BiomeType.WETLAND: ("≋", (60, 140, 120), (30, 100, 80))
        }
        ...
        for y in range(self.world_height):
            for x in range(self.world_width):
                ...
                char, fg, bg = biome_display.get(tile.biome, ("?", (255, 255, 255), (0, 0, 0)))
                
                # Modify for special features
                if tile.has_major_river and tile.is_land:
                    char = "≈"  # River
                    fg = (100, 150, 255)
...
if __name__ == "__main__":
    ...
    world_map = generator.generate_complete_world()
    generator.print_world_stats()
    
    # Display a small sample of the world (top-left 40x30)
    print(f"\nWorld Sample (40x30 from top-left):")
    for y in range(min(30, generator.world_height)):
        line = ""
        for x in range(min(40, generator.world_width)):
            tile = world_map[y][x]
            line += tile.char
        print(line)
    
    # Example of accessing specific tile data
    sample_tile = world_map[48][64]  # Center of 128x96 world
    print(f"\nSample tile at center ({sample_tile.x}, {sample_tile.y}):")
    print(f"  Elevation: {sample_tile.final_elevation:.1f}m")
    print(f"  Biome: {sample_tile.biome.value}")
    print(f"  Climate: {sample_tile.climate_zone.value}")
    print(f"  Temperature: {sample_tile.temperature:.2f}")
    print(f"  Precipitation: {sample_tile.precipitation:.2f}")
    print(f"  Is Land: {sample_tile.is_land}")
    print(f"  Has River: {sample_tile.has_major_river}")
    print(f"  Plate ID: {sample_tile.plate_id}")